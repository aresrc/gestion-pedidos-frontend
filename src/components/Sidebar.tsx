...
const Sidebar = () => {
  ...
  const [openComprobanteSubMenu, setOpenComprobanteSubMenu] = useState(false);
  ...
  const navItems = [
    ...
    },
    {
      name: 'Comprobantes',
      subItems: [
        { name: 'Emitir Comprobante', href: '/dashboard/comprobante/emitir' },
  
      ],
      openState: openComprobanteSubMenu,
      setOpenState: setOpenComprobanteSubMenu,
    },
    { name: 'Perfil', href: '/dashboard/profile' },
    { name: 'Configuración', href: '/dashboard/settings' },
    { name: 'Reporte de Ventas', href: '/dashboard/reportes' },
  ];
                ...
                <>
                  <button
                    ...
                    className={`w-full flex justify-between items-center py-2 px-4 rounded hover:bg-gray-700 ${
                      item.subItems.some(subItem => pathname.startsWith(subItem.href)) ? 'bg-gray-700 font-semibold' : ''
                    }`}
                  >
                    <span>{item.name}</span>
                    {item.openState ? (
